REM 
REM This script outputs the result of a query in text-display-friendly format, i.e. one or multiple lines per column
REM It is based on the PRINT_TABLE procedure which used to be available at asktom.com 
REM 

def input_query="SELECT * FROM &1 WHERE &2 = '&3' "
SET SERVEROUT ON 
...
DECLARE 
    p_query    CONSTANT  VARCHAR2(32000) := q'[&input_query]';
    p_date_fmt   VARCHAR2(100) := 'dd-mon-yyyy hh24:mi:ss';
...
-- are just printing them out
    FOR i IN 1..l_colcnt LOOP
       -- dbms_output.put_line( 'col_name/typ:'|| l_desctbl(i).col_name|| ' / '|| l_desctbl(i).col_type );
       IF          l_desctbl(i).col_type NOT IN (       109 /*collection*/  ,  113                )
       THEN
...
-- so we'll only see the first 200 characters by my design…
    WHILE ( dbms_sql.fetch_rows(l_thecursor) > 0 ) LOOP
        l_row_no := l_row_no + 1; 
        dbms_output.put_line('======================================= RECORD '||l_row_no ||' '|| to_char( sysdate, p_date_fmt ));
        FOR i IN 1..l_colcnt LOOP
            IF                    l_desctbl(i).col_type NOT IN ( 109, 113      )
            THEN
    ...
    END LOOP;
    dbms_output.put_line('======================================= END OF OUTPUT ' || to_char( sysdate, p_date_fmt ));
